(function() {
  ...
  nav.innerHTML =
    '<div style="position:fixed;bottom:20px;right:20px;z-index:9999;font-family:Segoe UI,sans-serif;display:flex;gap:8px;align-items:flex-end">' +
      ...
      '<div style="position:relative">' +
        ...
        '<div id="game-menu" style="display:none;position:absolute;bottom:50px;right:0;background:#fff;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.3);min-width:280px;max-height:400px;overflow-y:auto;padding:8px 0"></div>' +
      '</div>' +
    '</div>';
  ...
  games.forEach(function(g) {
    ...
    a.style.cssText = 'display:block;padding:10px 16px;text-decoration:none;color:' + (isCurrent ? '#27ae60' : '#2c3e50') + ';font-size:.9em;font-weight:' + (isCurrent ? 'bold' : 'normal') + ';border-left:3px solid ' + (isCurrent ? '#27ae60' : 'transparent');
